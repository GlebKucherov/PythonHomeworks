...
# В случае, если данные перекрываются, то идеальной границы не существуют. У модели существует гиперпараметр, который определяет "размытие" отступа
...
for i in range(2):
    for j in range(4):
        ...
        # Если C большое, то отступ задаётся "жёстко". Чем меньше C, тем отступ становится более "размытым"
...
# - Большая зависимость от параметра размытости C. Поиск может привести к большим вычислительным затратам
